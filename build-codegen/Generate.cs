...
namespace RocksDbPrepareCApiHeader
{
    class Generate
    {
        ...
        private static string ManualEnumName(NativeEnum nativeEnum)
            ...
            // Every value here begins with "write_", so the guess keeps that word in the
            // type name and strips it from the members, giving TxndbWritePolicyWrite.Committed.
            nativeEnum.Values.Any(v => v.Name == "rocksdb_txndb_write_policy_write_committed") ? "TxndbWritePolicy" :
            "";

        /* Callbacks that c.h gives a name of its own and that a caller has to be able to
         * supply from managed code, so they are declared as real delegates instead of
         * being marshalled as the opaque pointers every other named callback gets.
         */
        private static readonly string[] ManualCallbackDelegates = new[]
        {
        ...
        private static readonly HashSet<string> ManualCallbackDelegateNames =
            ManualCallbackDelegates.Select(d => Regex.Match(d, @"([a-zA-Z0-9_]+)\s*\(").Groups[1].Value).ToHashSet();
        ...
        static async Task ProcessAsync()
        {
            ...
            // Keyed by position rather than by title: rocksdb's c.h repeats a region
            // heading now and then (11.6.0's generated additions gave the header a
            // second "/* BackupEngineOptions */"), and a title keyed lookup turns that
            // into either a duplicate key or a section quietly emitted twice.
            var managedFunctions = regions
                .Select(r => (Region: r, Functions: GetManagedFunctions(r).ToArray()))
                .ToList();
            ...
            // Callbacks the header gives a name of their own ("typedef int (*rocksdb_x_cb)(...)")
            // arrive as ordinary type names rather than as the inline function pointers the
            // delegate generation below handles, so each one needs an alias to marshal
            // through. They were listed by hand until 11.6.0 added five more of them.
            foreach (var callbackTypeDef in ParseNativeCallbackTypeDefNames(modified).Where(n => !ManualCallbackDelegateNames.Contains(n)))
                nativeRawCs.AppendLine($"using {callbackTypeDef}  = System.IntPtr;");
            ...
            //Temporary fix for code generation for typedefs not currently being recognized
            foreach (var declaration in ManualCallbackDelegates)
                nativeRawCs.AppendLine(declaration);

            nativeRawCs.AppendLine("#endregion");

            foreach (var (region, functions) in managedFunctions.Where(rmf => (rmf.Region.NativeEnums.Length + rmf.Region.NativeFunctions.Length) > 0))
            {
                ...
                nativeRawCs.StartBlock("{");
                foreach (var managedFunction in functions)
                {
        ...
        {
            // Cut the common prefix back to a word boundary rather than dropping it when
            // it does not land on one. rocksdb_wal_file_type_{archived,alive}_log share
            // "rocksdb_wal_file_type_a", and discarding that leaves only the "_log"
            // suffix to name the enum by, which produces a type called "Log".
            var commonPrefix = GetCommonPrefix(nativeEnum.Values.Select(e => e.Name).ToArray()).TrimAfterLast('_').If(p => p.EndsWith("_"));
            ...
            // An enum whose values share neither a prefix ending in "_" nor a suffix
            // starting with one leaves both of these null, so the prefix branch has to
            // test for a prefix rather than only for it being the longer of the two.
            var guessedEnumName =
                ...
                "";
            // The manual name comes first: it is only ever set for an enum whose guessed
            // name is wrong, which is not always the same thing as there being no guess.
            var enumName = ManualEnumName(nativeEnum).OrElse(nativeEnum.Name).OrElse(guessedEnumName).OrElse("(Unable To Guess Name)");
        ...
        // "typedef void (*rocksdb_create_backup_options_progress_cb)(void* state);", possibly
        // wrapped over several lines. The name is the only part that matters here: the
        // pointer is marshalled as an IntPtr whatever it points at.
        static Regex NativeCallbackTypeDefPattern { get; } = new Regex(@"typedef\s[^;]*?\(\s*\*\s*([a-zA-Z0-9_]+)\s*\)\s*\(", RegexOptions.Compiled | RegexOptions.Singleline);
        static IEnumerable<string> ParseNativeCallbackTypeDefNames(string source)
            => NativeCallbackTypeDefPattern.Matches(source).AsEnumerable().Select(m => m.Groups[1].Value).Distinct();
        ...
        static IEnumerable<NativeEnum> ParseNativeEnumerations(string source)
        {
            ...
            foreach (var match in matches.AsEnumerable())
            {
                ...
                // The values below are only read as plain decimal, so fold the two other
                // spellings rocksdb writes them in down to that first: hexadecimal
                // literals, and shifts used to spell out bitmask flags. The trace filter
                // enum added in 11.6.0 combines both ("rocksdb_trace_filter_get = 0x1 << 0"),
                // and anything left unfolded here parses as a value of its own and
                // corrupts the whole enumeration rather than just that one entry.
                body = Regex.Replace(body, @"\b0[xX]([0-9a-fA-F]+)\b", m => Convert.ToInt64(m.Groups[1].Value, 16).ToString());
                body = Regex.Replace(body, @"\b([0-9]+)\s*<<\s*([0-9]+)\b", m => ((long)int.Parse(m.Groups[1].Value) << int.Parse(m.Groups[2].Value)).ToString());
    ...
    static class Extensions
    {
        ...
        /// <summary>Everything up to and including the last occurrence of <paramref name="separator"/>, or the empty string when there is none.</summary>
        public static string TrimAfterLast(this string input, char separator)
            => string.IsNullOrEmpty(input) ? input : input.Substring(0, input.LastIndexOf(separator) + 1);
